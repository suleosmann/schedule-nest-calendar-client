...
function EventDetailsModal({ event, closeModal }) {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="float-right border border-gray-400 rounded-full h-6 w-6 items-center justify-center cursor-pointer "
          onClick={closeModal}
        >
        ...
        <div className="mt-2 bg-green-300 rounded-xl ">
          ...
          <div className="flex-row flex justify-center  items-center text-base  text-gray-900 ">
            <div className="mr-2">
              <p> Start Time</p>
              <p>{event.start.toString().slice(16, 24)}</p>
            </div>
            <div className="ml-2">
              <p> End Time</p>
              <p>{event.end.toString().slice(16, 24)}</p>
            </div>
          </div>
        </div>
      ...
      {isEditModalOpen && (
        <EditEventModal event={event} closeModal={closeEditModal} />
      )}
      {isDeleteModalOpen && (
        <DeleteEventModal
          type="deletion"
